...
DMatrix* DMatrix::Load(const std::string& uri,
  ...
  // 参数传入为：1）文件URI地址，
  //            2）silent开关，为true打印统计信息，
  //            3）load_row_split为分布式开关，为true，则对数据进行分片shard。
  //            4）file_format为数据解析格式，默认参数为"auto"，自动解析文件数据格式。
  ...
  // 若是则LoadBinary直接初始化SimpleCSRSource数据源对象source。
  ...
  //  4. 构建解析器parser，基于工厂设计模式，基类dmlc::Parser<uint32_t>调用静态方法Create(), 尽管格式解析为”auto”，
  // 目前使用libsvm格式解析。
  ...
  // 6. 根据参数slient打印相关统计信息，尝试读取.group后缀的文件；
  // .base_margin后缀的boost初始设定值；.weight样本的权重，用于代价敏感学习，不存在则跳过。
