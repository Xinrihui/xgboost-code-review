...
class CLI {
  ...
  void CLITrain() {
    ...
    //rabit::IsDistributed()判断是否为分布式模式，若是，则打印相关log信息。
    ...
    // DMatrix::Load()根据数据文件路径，支持本地路径与分布式的hadoop文件路径，
    // 生成统一URI路径格式；解析底层数据格式，支持libsvm、libffm。
    // 解析生成数据源对象，再加载到内存数据CSR格式对象，即DMatrix对象。
    // 加载训练数据与预测数据到dtrain、deval（支持多份eval数据），并合并到cache_mats，
    // cache_mats用于构建全局特征统计信息，合并是因为deval中的数据可能超过dtrain的边界，或者dtrain不存在。
    // 接着将dtrain加入deval，因为训练集也需要评估。

    std::shared_ptr<DMatrix> dtrain(DMatrix::Load(  // 读取数据
        param_.train_path,
    ...
    // 使用cache_mats实例化learner，基于C++工厂设计模式，使用Learner::Create()静态方法实例化出具体实现类LearnerImpl。
    // Learner 继承于rabit::Serializable，分布式下具备相互通信功能。
    ...
    for (int i = version / 2; i < param_.num_round; ++i) {
      ...
      if (version % 2 == 0) {
        LOG(INFO) << "boosting round " << i << ", " << elapsed
                  << " sec elapsed";
        learner_->UpdateOneIter(i, dtrain);  // 循环更新
