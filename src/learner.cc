...
class LearnerImpl : public LearnerIO {
 public:
  ...
  void UpdateOneIter(int iter, std::shared_ptr<DMatrix> train) override {
    ...
    this->PredictRaw(train.get(), &predt, true, 0, 0); // 计算当前样本的预测结果
    TrainingObserver::Instance().Observe(predt.predictions, "Predictions");
    monitor_.Stop("PredictRaw");

    monitor_.Start("GetGradient");
    obj_->GetGradient(predt.predictions, train->Info(), iter, &gpair_); // 计算 一阶梯度 和 二阶梯度
    monitor_.Stop("GetGradient");
    TrainingObserver::Instance().Observe(gpair_, "Gradients");

    gbm_->DoBoost(train.get(), &gpair_, &predt); // Boost操作 ：DoBoost
